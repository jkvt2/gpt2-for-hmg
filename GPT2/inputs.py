...
from data_utils import load_normalised_examples

# Expects .tfrecords files as produced by the script in datasets in a google storage bucket

def hmr(params, eval=False, batch=True):
    trn_ex, _ = load_normalised_examples(
    ...
    if eval:
        files = trn_ex[:params["batch_size"]]
    else:
        files = trn_ex
        
    def _sample_text(x):
        s = tf.shape(x)[0]
        r = tf.random.uniform([], maxval=s-amount, dtype=tf.dtypes.int32)
