...
def model(X, params, labels=None, past=None, scope='model', reuse=False, train=False):
    with tf.variable_scope(scope, reuse=reuse):
        results = {}
        print(X)
        batch, sequence, feats = shape_list(X)
        ...
        wte = tf.layers.dense(
            inputs=X + train * tf.random.normal(tf.shape(X), 0, stddev=params["train_noise"]),
            units=params["n_embd"])
        wte = dropout(wte, params["embed_dropout"], train)
        h = wte + tf.gather(wpe, positions_for(X, past_length))
        # Transformer
        presents = []
        ...
        flat_pred = tf.layers.dense(inputs=h, units=85*n_bins*2)
        results['pred'] = tf.reshape(flat_pred, (batch, -1, 85, n_bins, 2))
